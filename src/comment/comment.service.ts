...
export class CommentService {
  ...
  dataFilter(comment: Comment) {
    const filterUser = (user) => {
      let { id, password, salt, roles, deleted, email, birthday, create_time, update_time, ...reset } = user;
      return reset;
    };
  ...
  async create(comment: CreateCommentDto) {
    ...
    const parentComment = comment.parentCommentId ? await this.findOne(comment.parentCommentId) : null;
    if (parentComment && parentComment.post.id !== comment.postId) {
      throw new HttpException("新评论与父级评论的文章id不一致", HttpStatus.BAD_REQUEST);
    }
  ...
  async findAll(options: FindAllCommentsDto) {
    const { page = 1, pageSize = 10, postId, username, startCreateTime, endCreateTime, ...queryConditions } = options;
    ...
    if (postId) {
      query.where("comment.parentComment IS NULL")
  ...
  async delete(commentId: number) {
    ...
    if(comment.parentComment && comment.parentComment.replies) {
      comment.parentComment.replies = comment.parentComment.replies.filter(reply => reply.id !== comment.id);
